...
import { ChevronRight, Plus, Minus, ShoppingCart, CreditCard } from "lucide-react";
...
const ProductDetailClient = ({ id }: { id: string }) => {
  ...
  useEffect(() => {
    const fetchProductDetail = async () => {
      if (!id) return;
      try {
        const response = await fetch(`${API_URL}/get_product_detail.php?id=${id}`);
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`relative w-24 aspect-square rounded-xl overflow-hidden flex-shrink-0 transition-all border-2 ${selectedImage === img.image_path ? "border-brand-primary" : "border-transparent"
                    }`}
                >
                  <Image
                    src={img.image_path}
          ...
          <div className="flex justify-center gap-12 border-b border-gray-100">
            {tabs.map((tab) => (
              <button
                ...
                className={`pb-6 text-lg font-bold transition-all relative ${activeTab === tab.id ? "text-brand-primary" : "text-gray-300 hover:text-gray-500"
                  }`}
              >
                {tab.title}
                {activeTab === tab.id && (
